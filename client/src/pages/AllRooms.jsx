...
const AllRooms = () => {
    ...
    //function to sort the rooms based on the selected sort option
    const sortRooms = (a, b) => {
        ...
        <div>
            ...
            {filteredRooms.map((room)=> (
                <div className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0' key={room._id}>
                    <img src={room.images[0]} alt="hotel-img" title='View Rooms Details'className='max-h-65 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer' onClick={()=> {navigate(`/rooms/${room._id}`); scrollTo(0,0)}} />
                    <div className='md:w-1/2 flex flex-col gap-2'>
                        ...
                        <div className='flex items-center gap-1text-gray-500 mt-2 text-sm'>
                        ...
                        <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                            {room.amenities.map((item, index)=> (
                                <div key={index} className='flex items-center gap-2 bg-[#F5F5FF]/70 px-2 py-2 rounded-lg'>
            ...
            <div className={`${openFilters ? 'h-auto' : "h-0 lg:h-auto"} overflow-hidden transition-all duration-700`}>
                ...
                <div className='px-5 pt-5 pb-7'>
                    ...
                    {sortByOptions.map((option, index)=> (
                        <RadioButton key={index} label={option} selected={sortBy === option} onChange={()=> handleSortChange(option)} />
                    ))}
                </div>
            </div>
        </div>
